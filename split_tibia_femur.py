...
def knee_joint_z_index(limb, min=1400, max=2300, threshold=120):
    ...
    threshold_idx = [np.vstack(np.nonzero(i>threshold)) for i in limb]
    index, momentum =zip(*[(i,np.std(j, axis=1)) for i ,j in enumerate(threshold_idx) if j.size>10000])
    ...
    momentum = np.convolve(momentum, np.ones(10)/10)
    if max > momentum.shape[0]:
        max = np.argsort(momentum)[-1]
    z_index = np.argsort(momentum[min:max])[0]+min
...
if __name__ == "__main__":
    masterfolder = r'C:\Users\murtaza\Desktop\07.31.21_Obese\Reconstructed Image\Week 0_Post Surgery'
    masterout = r'C:\Users\murtaza\Desktop\07.31.21_Obese\Shubo-Tiankuo\shubo tibia femur split\Week 0'
